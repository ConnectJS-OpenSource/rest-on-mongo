...
const jsonBodyParser = require('./documentFieldDefinitions');
const qpm = require('query-params-mongo');
const {processJsonFields} = require("./documentFieldDefinitions");

const processQuery = qpm({
  autoDetect: [
      { fieldPattern: /_id$/, dataType: 'objectId' },
      { fieldPattern: /_on$/, dataType: 'date' }
  ],
  converters: {
...
async function handleCreateOne(req, res, next) {
  try {
    const doc = processJsonFields(req.body.data, req.body.fieldsSpec);
    doc._id = req.id;
...
async function handleCreateMany(req, res, next) {
  try {
    const docs = Array.isArray(req.body.data) ? req.body.data.map(m => processJsonFields(m, req.body.fieldsSpec)) : [req.body.data];
...
async function handleGetCollections(req, res, next){
  ...
  try{
    const cursor = await req.db.listCollections();
    const collections = (await cursor.toArray()).map(m => m.name);
...
async function handleUpdateOne(req, res, next) {
  try {
    ...
    const body = jsonBodyParser.processJsonFields(req.body.data, req.body.fieldsSpec);
    const response = await req.collection.updateOne({ _id: req.id }, { $set: body });
...
async function handleUpdateMany(req, res, next) {
  try {
    const ops = [];
    for (let i = 0; i < req.body.data.length; i += 1) {
      const object = processJsonFields(req.body.data[i], req.body.fieldsSpec);
      const { _id } = object;
      if (!_id) {
        res.status(400).send({
          status: 'error',
...
async function handleReplaceOne(req, res, next) {
  try {
    if(req.id)
      delete req.body.data._id;
    const body = jsonBodyParser.processJsonFields(req.body.data, req.body.fieldsSpec);
    const response = await req.collection.replaceOne({ _id: req.id }, body);
    const { result, matchedCount, modifiedCount } = response;
    if (matchedCount !== 1) {
      res.status(404);
    }
...
async function handleReplaceMany(req, res, next) {
  try {
    const ops = [];
    for (let i = 0; i < req.body.data.length; i += 1) {
      const object = jsonBodyParser.processJsonFields(req.body.data[i], req.body.fieldsSpec);
      const { _id } = object;
      if (!_id) {
        res.status(400).send({
          status: 'error',
